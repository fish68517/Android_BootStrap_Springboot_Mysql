package com.archive.app.tts;

import static com.archive.app.view.fragment.SettingsFragment.KEY_VOICE_THEME_POSITION;
import static com.archive.app.view.fragment.SettingsFragment.PREFS_NAME;
...
import com.archive.app.asr.SpinnerAdapter;
...
public class TTSActivity extends AppCompatActivity implements SeekBar.OnSeekBarChangeListener,
    ...
    private void initView(){
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            schedule = getIntent().getSerializableExtra("TTS_Bean",Schedule.class);
        }
        if(schedule!=null){
            mEd_Text.setText(schedule.getTitle() + "," + schedule.getDescription() + ","
                + schedule.getStartTime() + "," + schedule.getEndTime() + "," + schedule.getLocation());
        }
    ...
    private String loadSettings() {
        ...
        Log.d(TAG, "voiceThemePosition = " + voiceThemePosition);
        String[] voiceThemes = getResources().getStringArray(R.array.voice_themes);
        if (voiceThemePosition >= 0 && voiceThemePosition < voiceThemes.length) {
            Log.d(TAG, "voiceTheme = " + voiceThemes[voiceThemePosition]);
            return voiceThemes[voiceThemePosition];
        } else {
            Log.d(TAG, "Invalid voiceThemePosition: " + voiceThemePosition);
            return voiceThemes[0]; // 默认值
        }
    ...
    private void start(){
        ...
        text = mEd_Text.getText().toString();
        Log.d(TAG,"text = " + text);
        // text = 美女男,null,Wed Aug 27 16:58:24 GMT+08:00 2025,null,你的牛奶吃没吃
        // 根据日志 过滤 null  将 Wed Aug 27 16:58:24 GMT+08:00 2025 转换成 yyyy-mm-dd hh:min:ss
       ...
       /* Log.d(TAG,"text after= " + text);
        ...
        // 获取设置的语音主题
        String voiceTheme =loadSettings();
        if(voiceTheme.equals("讯飞小燕")){
            mOnlineTTS.vcn("x4_xiaoyan");
        }else if(voiceTheme.equals("讯飞小露")){
            mOnlineTTS.vcn("x4_yezi");
        }else if(voiceTheme.equals("讯飞许久")){
            mOnlineTTS.vcn("aisjiuxu");
        }else if(voiceTheme.equals("讯飞小婧")){
            mOnlineTTS.vcn("aisjinger");
        } else if (voiceTheme.equals("讯飞许小宝")){

            mOnlineTTS.vcn("aisbabyxu");
        } else {
            mOnlineTTS.vcn("x4_xiaoyan");
        }
...
//            mOnlineTTS.stop();
    ...
    public void onClick(View v) {
        int id = v.getId();
        if (id == R.id.ai_tts_play_btn) {
            start();
        } else if (id == R.id.ai_tts_stop_btn) {
            stop();
        }
...
/*        switch (v.getId()){
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        int idq = parent.getId();
        if (idq == R.id.ai_tts_vcn_spinner) {
            mTTSParams.vcn = TTSParams.getVCN().get(position).value;
            Log.d(TAG, "vcn = " + mTTSParams.vcn);
        }
   /*     switch (parent.getId()) {
